...
import com.zhudong.tianyuekotlin.net.NewsUtils
import com.zhudong.tianyuekotlin.ui.adapter.NewsDetailAdapter
import com.zhudong.tianyuekotlin.ui.base.BaseFragment
import com.zhudong.tianyuekotlin.ui.news.contract.DetailContract
import com.zhudong.tianyuekotlin.ui.news.presenter.DetailPresenter
...
import com.zhudong.tianyuekotlin.widget.NewsDelPop
    ...
    private fun bannerToRead(itemBean: NewsDetail.ItemBean?) {
        itemBean?.let {
            when (it.type) {
                NewsUtils.TYPE_DOC -> {
                    it.documentId?.let {
                        activity?.let { it1 -> ArticleReadActivity.launch(it1, it) }
                    }
                }
                NewsUtils.TYPE_SLIDE -> activity?.let { it1 -> ImageBrowseActivity.launch(it1, it) }
                NewsUtils.TYPE_ADVERT -> it.link?.weburl?.let { activity?.let { it1 -> AdvertActivity.launch(it1, it) } }
    ...
    private fun toRead(itemBean: NewsDetail.ItemBean?) {
        itemBean?.let {
            when (it.itemType) {
                ...
                NewsDetail.ItemBean.TYPE_SLIDE -> activity?.let { it1 -> ImageBrowseActivity.launch(it1, it) }
                NewsDetail.ItemBean.TYPE_ADVERT_TITLEIMG, NewsDetail.ItemBean.TYPE_ADVERT_SLIDEIMG, NewsDetail.ItemBean.TYPE_ADVERT_LONGIMG -> {
                    it.link?.weburl?.let { activity?.let { it1 -> AdvertActivity.launch(it1, it) } }
                }
